...
meanDegs = c(1, 3, 10)
coefs = seq(-.5, .5, len = 4)
decays = 10^(seq(-1, .5, len = 5))  # May want better resolution here. 
            ...
            parLapply(clust, coefs, function(dp) {
              ...
              decaysWithinCoef = 
                lapply(decays, function(decay) {
                  ...
                  message("\nAt ", Sys.time(), ". coef = ", round(dp, 2), 
                          ", decay = ", round(decay, 2), ", sim'ing on ", simParam,
                          ", edges constrained: ", constrained, ", theta fixed: ", fixed)
